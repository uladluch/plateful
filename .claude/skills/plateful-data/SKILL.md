...
## Кроул сетей — отдельный скилл

Как обновлять числа с сайта сети: `plateful-crawl`. Там порядок работы,
запреты (неполная этикетка не применяется, битая позиция не затирает верную),
матчинг имён и правила вежливости. Здесь — только то, что кроул трогает:
`items` получает новую версию позиции, старая закрывается `valid_to`.
