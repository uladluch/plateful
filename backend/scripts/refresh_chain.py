...
Четыре шага подряд, каждый уже умеет работать сам по себе:
  1. кроул — свежие числа с сайта сети;
  2. присутствие в меню — что сеть продаёт сегодня, а что сняла;
  3. снимки — официальная съёмка для тех блюд, у которых её ещё нет;
  4. публикация пака — если попросили.
...
def refresh(chain: str) -> bool:
    # Кроул первым: остальные шаги описывают позиции, а он их обновляет.
    # Он держит сам себя — при held в базу не пишет и возвращает 1, — поэтому
    # дальше идём в любом случае, но результат считаем неполным.
    ok = run(f"{chain}: свежие числа с сайта",
             [str(SCRIPTS / "crawl_chain.py"), chain, "--apply"])
